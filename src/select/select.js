const getTemplate = (options = {}) => {
  ...
  const items = data.map((item) => {
    ...
    return `
      <li class="select__item ${selected}" data-type="item" data-id="${item.id}">${item.value}</li>
    `
  })
...
export class Select {
  ...
  select(id) {
    ...
    this.$el.querySelectorAll('[data-type="item"].selected').forEach((el) => {
